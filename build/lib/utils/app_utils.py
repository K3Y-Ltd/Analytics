...
def cv_customized(X: pd.DataFrame, y: pd.Series, kf: KFold, models: List[BaseEstimator], metric: str, folds: int) -> tuple[
    Any, Series]:
    """
    ...
    cv_results = np.zeros((len(models), folds, 5))
    ...
    for model_idx, model in enumerate(models):
        ...
        meta_data = pd.concat([meta_data, pd.DataFrame([new_row])], ignore_index=True)
    ...
    return best_model, best_config, meta_data
...
def model_selection(X: pd.DataFrame, y: pd.Series, folds: int, metric:str) -> Dict[str, Dict[str, Union[BaseEstimator, DataFrame]]]:
    ...
    lr_param_grid = {'penalty': ['l1'], 'C': [0.5, 1.0, 10.0], 'solver': ['liblinear'], 'max_iter': [100]}
    ...
    svm_param_grid = {'C': [1, 10], 'penalty': ['l1'], 'loss': ['squared_hinge'], 'dual': [False],
                      'max_iter': [200]}
